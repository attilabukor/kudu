...
static Status ParseTableIdentifier(const string& table_name, string* db, string* table) {
  Slice tbl;
...
Status RangerAuthzProvider::AuthorizeAlterTable(const string& old_table,
  ...
  if (old_table == new_table) {
    ...
    if (PREDICT_FALSE(!authorized)) {
      LOG(WARNING) << Substitute("User $0 is not authorized to DROP $1", user, old_table);
      return Status::NotAuthorized(kUnauthorizedAction);
    }
