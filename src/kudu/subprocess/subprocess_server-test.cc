...
static const string kHello = "hello world";
...
class SubprocessServerTest : public KuduTest {
 public:
  ...
  // Reset the subprocess server with to account for any new configurations.
  Status ResetSubprocessServer(int java_queue_size = 0,
                               int java_writer_threads = 0) {
    ...
    vector<string> argv = {
      ...
      "-jar", Substitute("$0/kudu-subprocess-echo.jar", subprocess_home)
    };
    if (java_queue_size > 0) {
      argv.emplace_back("q");
      argv.emplace_back(std::to_string(java_queue_size));
    }
    if (java_writer_threads > 0) {
      argv.emplace_back("w");
      argv.emplace_back(std::to_string(java_writer_threads));
    }
...
TEST_F(SubprocessServerTest, TestTimeoutWhileQueueingCalls) {
  ...
  // Since we slept for less than the timeout, we should've seen some
  // successes.
  ASSERT_TRUE(has_success) << "expected at least one success";

  // But we also sent a ton of requests and should've overwhelmed the pipe and
  // our outbound queue.
